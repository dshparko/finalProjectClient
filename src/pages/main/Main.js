...
function Main(props) {
    ...
    const send = async () => {
        try {
            let arr = []
            const res = await axios.get(`finalprojectserver-production.up.railway.app/auth/search/${workTitle}`)
    ...
    const searchPost = async (item) => {
        try {
            ...
            setTags(item)
            alert(item)
            const res = await axios.get(`finalprojectserver-production.up.railway.app/auth/searchTag/${item}`)
